...
export class ManagerUsersComponent implements OnInit {
  ...
  manageableRoles = ['Client', 'Trainer', 'Receptionist'];
  ...
  private createUser() {
    ...
    this.userService.createUser(userData).subscribe({
      ...
      error: (err) => {
        const errorMessage = err.error?.errors?.Email?.[0] || err.error?.errors?.Role?.[0] || err.error?.errors?.CreateUser?.[0] || 'Failed to create user';
  ...
  private updateUser() {
    ...
    this.userService.updateUser(this.editingUserId, userData).subscribe({
      ...
      error: (err) => {
        const errorMessage = err.error?.errors?.Email?.[0] || err.error?.errors?.Role?.[0] || err.error?.errors?.UpdateUser?.[0] || 'Failed to update user';
        this.messageService.add({ severity: 'error', summary: 'Error', detail: errorMessage });
      }
    });
  }
